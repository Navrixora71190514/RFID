...
void setup() {
    ...
    Serial.println("RFID Tambah Saldo");

    Serial.println();

    Serial.println("Data di simpan pada RFID di sector #1 blocks #4");
}
...
void loop() {
  if (Serial.available()){
    ...
    if (saldo > 255){
      saldo = 0;
      Serial.println("input Saldo tidak boleh lebih dari 255");
    }
    if (saldo < 0){
      saldo = 0;
      Serial.println("input Saldo tidak boleh kurang dari 0");
    }
    digit = saldo;
    saldo *= 1000;
  ...
  if (notif){
    ...
    Serial.println("tiap input angka dikali 1.000");
    Serial.println("contoh input 9 untuk saldo 9ribu || max input saldo 255rb");
    Serial.println();
    Serial.println("==============TAP UNTUK CEK SALDO=================");
  }
  ...
  if (isiSaldo){
    ...
    if (status != MFRC522::STATUS_OK) {
        Serial.println("Kartu RFID Tidak terbaca");
    ...
    if (buffer[0] == dataBlock[0]){
      ...
      Serial.println(saldo);
      Serial.println("====================ISI SALDO BERHASIL===================");
    }else{
